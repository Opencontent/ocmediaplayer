...
{def $cover_image_class = ezini( 'DefaultSettings', 'VideoCoverImageClass', 'ocmediaplayer.ini' )
     ...
     $file = concat("content/download/",$attribute.contentobject_id,"/",$attribute.content.contentobject_attribute_id,"/",$attribute.content.original_filename|slugize()|explode('-')|implode('.'))|ezurl
     $cover = false()
     $advert = false()
     $captions = false()}
    ...
    {switch match=$key}
        ...
        {case match='advert'}
            {if and( is_set( $object.data_map.$identifier ), $object.data_map.$identifier.data_type_string|eq( 'ezxmltext' ), $object.data_map.$identifier.has_content )}
                {set $advert = $object.data_map.$identifier.content.output.output_text|strip_tags()}
            {/if}
        {/case}
        {case match='captions'}
            {if and( is_set( $object.data_map.$identifier ), $object.data_map.$identifier.data_type_string|eq( 'ezbinaryfile' ), $object.data_map.$identifier.has_content )}
                {set $captions = concat("content/download/",$object.data_map.$identifier.contentobject_id,"/",$object.data_map.$identifier.content.contentobject_attribute_id,"/",$object.data_map.$identifier.content.original_filename|slugize()|explode('-')|implode('.'))|ezurl}
            {/if}
        {/case}
        {case}{/case}
    {/switch}
...
{* @TODO capire come pulire il testo *}
{set $advert = false()}

<div class="video">

    <a	class="player no-js-hide"
        href={concat("content/download/",$attribute.contentobject_id,"/",$attribute.content.contentobject_attribute_id,"/",$attribute.content.original_filename|slugize()|explode('-')|implode('.'))|ezurl}
    ...
    <div id="videocontrols-{$attribute.contentobject_id}" class="controls"></div>

{ezscript_require(array( 'ezjsc::jquery', ocmp('flowplayer','js'), ocmp('ipad','js') ) )}
{ezcss_require( 'controls-apple.css' )}
<style type="text/css">
#videocontrols-{$attribute.contentobject_id} .track{ldelim}width:{$width|sub(20)}px{rdelim}
</style>
